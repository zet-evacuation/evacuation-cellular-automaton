...
/**
 * This class calculates statistics for several cycles of an cellular automaton. The values are obtained by getting the
 * mean average over all cycles.
 *
...
public class MultipleCycleCAStatistic implements CellStatisticMethods {
    ...
    public double calculateAverageAverageSpeedForGroup(
        ...
        for (int i = 0; i < cycles.size(); i++) {
            ...
            } catch (GroupOfIndsNoValueBecauseAlreadySafeException e) {
                noOfNoValueExc++;
            }
        }
...
// needed for "Maximale Geschwindigkeit über Zeit"
    ...
    public double calculateAverageSafetyTimeForGroup(
        ...
        for (int i = 0; i < cycles.size(); i++) {
            ...
            } catch (GroupOfIndsNotSafeException e) {
                noOfNotSafeExc++;
            }
        }
    ...
    public ArrayList<Exit> getPlannedExit(ArrayList<Individual> individual, int t)
            throws OneIndNoPotentialException, IllegalArgumentException {
        ArrayList<Exit> accumulatedValue = new ArrayList<>((int) numberOfCycles);
        for (int i = 0; i < cycles.size(); i++) {
            accumulatedValue.add(cycles.get(i).getIndividualStatistic().getPlannedExit(individual.get(i), t));
        }
        return accumulatedValue;
    }

    //needed for "Ausgangsverteilung"
    public HashMap<String, Double> getTakenExit(ArrayList<ArrayList<Individual>> individual) {

        HashMap<String, Double> exitUtilization = new HashMap<>();
        Exit exit;
        for (int i = 0; i < cycles.size(); i++) {
            for (Individual indi : individual.get(i)) {
                try {
                ...
                if (!exitUtilization.containsKey(exit.getName())) {
                    ...
                    exitUtilization.put(exit.getName(), exitUtilization.get(exit.getName()) + 1);
                }
            }
        }
